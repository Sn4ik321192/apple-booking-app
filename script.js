...
const pages = {
  ...
  myBookings: document.getElementById("myBookingsPage"),
...
async function openMyBookingsPage() {

  await fetchBookings();

  showPage("myBookings", 3);

  loadMyBookings();
}
function openProfilePage() { showPage("profile", 4); }
...
function renderPhoneState(){
  ...
  const button =
    document.getElementById("phoneActionBtn");

  const savedView =
    document.getElementById("savedPhoneView");

  const savedText =
    document.getElementById("savedPhoneText");

  if(!input || !button || !savedView || !savedText) return;
  ...
  if(userPhone){
    ...
    savedText.innerText = "📞 " + userPhone;

    button.innerText = "Изменить номер";
    ...
    button.innerText = "Сохранить номер";
  }
}
...
function handlePhoneButton(){
  ...
  if(userPhone && input.style.display === "none"){
 ...
 const myUsername =
  ...
  const myBookings =
    bookingsCache.filter(
      booking =>
        booking.telegram_username === myUsername
    );
...
if(booking.telegram_username !== myUsername){

  alert("Можно удалить только свою запись");
